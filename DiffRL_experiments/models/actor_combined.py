...
class ActorDeterministicMLP(nn.Module):
    ...
    def forward(self, observations, deterministic = False, modified=False):
        if(modified):
...
class ActorStochasticMLP(nn.Module):
    def __init__(self, obs_dim, action_dim, cfg_network, steps_num, device='cuda:0'):
        ...
        self.action_dim = action_dim
        self.obs_dim = obs_dim
        self.fixed_noise = torch.randn((self.steps_num, action_dim), dtype=torch.float32, device=device)
    ...
    def forward(self, obs, deterministic = False, modified=False, step=0):
        if(modified):
            obs = obs.detach()
        ...
        if deterministic:
            ...
            std = self.logstd.exp()
            # Use the fixed noise vector based on the current step for reproducibility
            noise = self.fixed_noise[step % len(self.fixed_noise)].unsqueeze(0).repeat(obs.size(0), 1)
            sample = mu + noise * std
            return sample
    
    '''def forward_with_dist(self, obs, deterministic = False, modified = False):
        if(modified):
            obs = obs.detach()
    ...
    def evaluate_actions_log_probs(self, obs, actions, modified=False):
        if(modified):
            obs = obs.detach()
