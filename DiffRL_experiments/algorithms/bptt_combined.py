...
class BPTT:
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        for i in range(self.steps_num):
            ...
            with torch.no_grad():
                raw_rew_original = rew_original.clone()
                raw_rew_modified = rew_modified.clone()
            ...
            done_env_ids_original = done_original.nonzero(as_tuple = False).squeeze(-1)
            done_env_ids_modified = done_modified.nonzero(as_tuple = False).squeeze(-1)
            ...
            if i < self.steps_num - 1:
                actor_loss_original = actor_loss_original + (- rew_acc_original[i + 1, done_env_ids_original]).sum()
                actor_loss_modified = actor_loss_modified + (- rew_acc_modified[i + 1, done_env_ids_modified]).sum()
            else:
                # terminate all envs at the end of optimization iteration
                actor_loss_original = actor_loss_original + (- rew_acc_original[i + 1, :]).sum()
                actor_loss_modified = actor_loss_modified + (- rew_acc_modified[i + 1, :]).sum()
        
            # compute gamma for next step
            gamma = gamma * self.gamma

            # clear up gamma and rew_acc for done envs
            gamma[done_env_ids_original] = 1.
            rew_acc_modified[i + 1, done_env_ids_modified] = 0.
            rew_acc_original[i + 1, done_env_ids_original] = 0.

            # collect episode loss
            with torch.no_grad():
                self.episode_loss -= raw_rew_original
                self.episode_discounted_loss -= self.episode_gamma * raw_rew_original
                self.episode_gamma *= self.gamma
                if len(done_env_ids_original) > 0:
                    self.episode_loss_meter.update(self.episode_loss[done_env_ids_original])
                    self.episode_discounted_loss_meter.update(self.episode_discounted_loss[done_env_ids_original])
                    self.episode_length_meter.update(self.episode_length[done_env_ids_original])
                    for done_env_id in done_env_ids_original:
    ...
    def train(self):
        ...
        def actor_closure():
            ...
            actor_loss_modified.backward(retain_graph=True)
