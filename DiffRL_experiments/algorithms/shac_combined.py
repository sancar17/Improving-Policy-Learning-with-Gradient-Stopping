...
import envs
import models.actor_combined
import models.critic_modified
...
class SHAC:
    def __init__(self, cfg):
        ...
        actor_fn = getattr(models.actor_combined, self.actor_name)
        self.actor = actor_fn(self.num_obs, self.num_actions, cfg['params']['network'], self.steps_num, device = self.device)
        critic_fn = getattr(models.critic_modified, self.critic_name)
    ...
    def compute_actor_loss(self, deterministic = False):
        rew_acc_original = torch.zeros((self.steps_num + 1, self.num_envs), dtype = torch.float32, device = self.device)
        rew_acc_modified = torch.zeros((self.steps_num + 1, self.num_envs), dtype = torch.float32, device = self.device)
        gamma = torch.ones(self.num_envs, dtype = torch.float32, device = self.device)
        next_values = torch.zeros((self.steps_num + 1, self.num_envs), dtype = torch.float32, device = self.device)
        
        actor_loss_original = torch.tensor(0., dtype = torch.float32, device = self.device)
        actor_loss_modified = torch.tensor(0., dtype = torch.float32, device = self.device)
        ...
        for i in range(self.steps_num):
            ...
            actions_original = self.actor(obs, deterministic = deterministic, modified=False, step=i)
            actions_modified = self.actor(obs, deterministic = deterministic, modified=True, step=i)

            joint_q = self.env.state.joint_q
            joint_qd = self.env.state.joint_qd

            obs_original, rew_original, done_original, extra_info_original = self.env.step(torch.tanh(actions_original))

            self.env.state.joint_q = joint_q
            self.env.state.joint_qd = joint_qd
            obs_modified, rew_modified, done_modified, extra_info_modified = self.env.step(torch.tanh(actions_modified))

            with torch.no_grad():
                raw_rew = rew_original.clone()
            
            # scale the reward
            rew_original = rew_original * self.rew_scale
            rew_modified = rew_modified * self.rew_scale
            
            if self.obs_rms is not None:
                # update obs rms
                with torch.no_grad():
                    self.obs_rms.update(obs_original)
                # normalize the current obs
                obs = obs_rms.normalize(obs_original)
            else:
                obs = obs_original
            ...
            if self.ret_rms is not None:
                # update ret rms
                with torch.no_grad():
                    self.ret = self.ret * self.gamma + rew_original
                    self.ret_rms.update(self.ret)
                    
                rew_original = rew_original / torch.sqrt(ret_var + 1e-6)
                rew_modified = rew_modified / torch.sqrt(ret_var + 1e-6)

            self.episode_length += 1
        
            done_env_ids = done_original.nonzero(as_tuple = False).squeeze(-1)

            next_values[i + 1] = self.target_critic(obs_original).squeeze(-1)

            for id in done_env_ids:
                if torch.isnan(extra_info_original['obs_before_reset'][id]).sum() > 0 \
                    or torch.isinf(extra_info_original['obs_before_reset'][id]).sum() > 0 \
                    or (torch.abs(extra_info_original['obs_before_reset'][id]) > 1e6).sum() > 0: # ugly fix for nan values
                    ...
                    if self.obs_rms is not None:
                        real_obs = obs_rms.normalize(extra_info_original['obs_before_reset'][id])
                    else:
                        real_obs = extra_info_original['obs_before_reset'][id]
            ...
            rew_acc_original[i + 1, :] = rew_acc_original[i, :] + gamma * rew_original
            rew_acc_modified[i + 1, :] = rew_acc_modified[i, :] + gamma * rew_modified

            if i < self.steps_num - 1:
                actor_loss_original = actor_loss_original + (- rew_acc_original[i + 1, done_env_ids] - self.gamma * gamma[done_env_ids] * next_values[i + 1, done_env_ids]).sum()
                actor_loss_modified = actor_loss_modified + (- rew_acc_modified[i + 1, done_env_ids] - self.gamma * gamma[done_env_ids] * next_values[i + 1, done_env_ids]).sum()
            else:
                # terminate all envs at the end of optimization iteration
                actor_loss_original = actor_loss_original + (- rew_acc_original[i + 1, :] - self.gamma * gamma * next_values[i + 1, :]).sum()
                actor_loss_modified = actor_loss_modified + (- rew_acc_modified[i + 1, :] - self.gamma * gamma * next_values[i + 1, :]).sum()
            ...
            rew_acc_original[i + 1, done_env_ids] = 0.
            rew_acc_modified[i + 1, done_env_ids] = 0.

            # collect data for critic training
            with torch.no_grad():
                self.rew_buf[i] = rew_original.clone()
                if i < self.steps_num - 1:
                    self.done_mask[i] = done_original.clone().to(torch.float32)
                else:
                    self.done_mask[i, :] = 1.
        ...
        actor_loss_original /= self.steps_num * self.num_envs
        actor_loss_modified /= self.steps_num * self.num_envs

        if self.ret_rms is not None:
            actor_loss_original = actor_loss_original * torch.sqrt(ret_var + 1e-6)
            actor_loss_modified = actor_loss_modified * torch.sqrt(ret_var + 1e-6)
            
        self.actor_loss = actor_loss_original.detach().cpu().item()
            
        self.step_count += self.steps_num * self.num_envs

        return actor_loss_original, actor_loss_modified
    ...
    def train(self):
        ...
        def actor_closure():
            ...
            actor_loss_original, actor_loss_modified = self.compute_actor_loss()
            self.time_report.end_timer("forward simulation")

            self.time_report.start_timer("backward simulation")
            actor_loss_original.backward(retain_graph=True)
            normal_gradients = {}
            ...
            '''Iterates through each parameter by name.
            Checks if there are gradients available from both the normal and modified backpropagation for this parameter.
            Creates a boolean tensor sign_agreement that holds True where the signs of the gradients agree and False where they don't.
            Initializes a new gradient tensor filled with zeros, which will be used to store the selectively modified gradients.
            Updates this new gradient tensor with values from combined_gradients where sign_agreement is True, meaning you only keep the gradients from combined_gradients where the signs agree with those from normal_gradients.
            Assigns the new, selectively modified gradient back to the parameter's .grad attribute for the optimizer to use in the update step.'''
            
            # Combine gradients
            #self.grad_norm_before_clip = tu.grad_norm(self.actor.parameters())
            for name, param in self.actor.named_parameters():
                if normal_gradients[name] is not None and combined_gradients[name] is not None:
                    sign_agreement = torch.sign(normal_gradients[name]) == torch.sign(combined_gradients[name])
                    new_grad = torch.zeros_like(param.grad)
                    new_grad[sign_agreement] = combined_gradients[name][sign_agreement]
                    param.grad = new_grad
                ...
                if normal_gradients[name] is None or combined_gradients[name] is None:
                    print("here")
